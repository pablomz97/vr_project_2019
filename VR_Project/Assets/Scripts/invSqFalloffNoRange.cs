...
//add this to a light source to use an actual inverse square falloff instead of the default range-attentuated version which is unrealistic, only works on baked lights
[ExecuteInEditMode]
public class invSqFalloffNoRange : MonoBehaviour
{
  public void OnEnable()
  {
    Lightmapping.RequestLightsDelegate testDel = (Light[] requests, Unity.Collections.NativeArray<LightDataGI> lightsOutput) =>
    {
      ...
      for (int i = 0; i < requests.Length; i++)
      {
        ...
        ld.falloff = FalloffType.InverseSquaredNoRangeAttenuation;
        lightsOutput[i] = ld;
      }
    };
